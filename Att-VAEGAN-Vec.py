...
trainLabelVectors = np.load(open(path+'out/trainDataWord2vec' , 'rb'))
trainAttribute = np.load(open(path+'out/trainAttributes' , 'rb'))
trainLabelGlove = np.load(open(path+'out/trainDataGlove' , 'rb'))
# trainLabelVectors=np.transpose(trainLabelVectors)
print("t")
print(trainLabelVectors)
print("a")
print(trainAttribute)
# trainLabelVectors=np.column_stack((trainLabelGlove,trainLabelVectors))
# trainLabelVectors=np.column_stack((trainLabelVectors,trainLabelGlove))
trainLabelVectors=trainLabelVectors
print(trainLabelVectors.shape)
print("ta")
print(trainLabelVectors)
testData = np.load(open(path+'out/testData','rb'))
testLabels = np.load(open(path+'out/testLabels','rb'))
testLabelVectors = np.load(open(path+'out/testDataWord2vec' , 'rb'))
testLabelGlove = np.load(open(path+'out/testDataGlove' , 'rb'))
testAttribute = np.load(open(path+'out/testAttributes' , 'rb'))
# testLabelVectors=np.transpose(testLabelVectors)
print(testLabelVectors.shape)
print(testAttribute.shape)
# testLabelVectors=np.column_stack((testLabelGlove,testLabelVectors))
# testLabelVectors=np.column_stack((testLabelVectors,testLabelGlove))
testLabelVectors=testLabelVectors
print(testLabelVectors.shape)
print(trainLabelVectors)
X_train = trainData
y_train=trainLabels
...
AWA1=loadmat('../../Datasets/xlsa17/data/AWA1/AWA.mat')
testClasses=AWA1["testclasses"]
trainClasses=AWA1["trainclasses"]

#获取所有的属性

Data = loadmat('../../Datasets/xlsa17/data/AWA1/att_word_splits.mat')
attr =Data["att"]
attr=np.transpose(attr)
Y_word2vec =Data["Y_word2vec"]
Y_word2vec=np.transpose(Y_word2vec)
# print(Y_word2vec)
# print("aaa")
# print(attr)
# print(attr.shape)
# print(Y_word2vec.shape)
Y_glove =Data["Y_glove"]
Y_glove=np.transpose(Y_glove)
# ATTR=np.column_stack((Y_word2vec,Y_glove))
#
# ATTR=np.column_stack((Y_word2vec,Y_word2vec))
ATTR=Y_word2vec
print(ATTR)
print(ATTR.shape)
...
#将生成的未见类数据存入mat文件
# import scipy.io
# scipy.io.savemat('pesudoTrainmy00.mat', mdict={'data': pseudoTrainData ,'labels': pseudoTrainLabels})
###########################################################
#训练分类器
#已经准备好了训练样本（x,y）
...
knn_clf_zsl=KNeighborsClassifier()
print(pseudoTrainData.shape)
knn_clf_zsl.fit(pseudoTrainData,pseudoTrainLabels)
print('Predicting...')
pred=knn_clf_zsl.predict(testData)
print(accuracy_score(testLabels,pred))
print(knn_clf_zsl.score(testData,testLabels))
...
# #############GZSL
# print('GZSL')
# knn_clf_gsl=KNeighborsClassifier()
# pseudoTrainData=np.array(pseudoTrainData)
# # (24790, 2048)
# # (2000, 2048)
# print(trainData.shape)
# print(pseudoTrainData.shape)
# #获得seen_AWA.mat，得到里面为train的部分
# seen_AWA1=loadmat('../../Datasets/xlsa17/data/AWA1/seen_AWA.mat')
# see_train_feature=seen_AWA1["see_train_feature"]
# seen_train_labels=seen_AWA1["seen_train_labels"]
# see_test_feature=seen_AWA1["see_test_feature"]
# seen_test_labels=seen_AWA1["seen_test_labels"]
# see_train_feature=np.array(see_train_feature)
# see_test_feature=np.array(see_test_feature)
# print(see_train_feature.shape)
# # print(see_test_feature)
# print(see_test_feature.shape)
# print(seen_train_labels)
# print(seen_test_labels)
# allTrainData=np.row_stack((pseudoTrainData,see_train_feature))
# # (2000,)
# # (24790, 1)
...
# # print(seen_train_labels)
# tl=[]
# for  i in seen_test_labels:
#     for j in i:
#         tl.append(j)
# seen_test_labels=tl
#
# tl=[]
# for  i in seen_train_labels:
#     for j in i:
#         tl.append(j)
# seen_train_labels=tl
# print(seen_train_labels)
# print(seen_test_labels)
#
# pseudoTrainLabels=np.array(pseudoTrainLabels)
# print(pseudoTrainData.shape)
# print(see_train_feature.shape)
# allTrainLabels=[list(pseudoTrainLabels),list(seen_train_labels)]
# # print(seen_train_labels)
# # seen_train_labels=list(seen_train_labels)
# # print(seen_train_labels)
# # tl=[]
# # for  i in allTrainLabels:
# #         tl.append(j)
# # allTrainLabels=tl
# allTrainLabels = [ i for item in allTrainLabels for i in item]
# print(allTrainLabels)
#
# # print(allTrainData.shape)
# print(allTrainData.shape)
# knn_clf_gsl.fit(allTrainData,allTrainLabels)
# print('Predict unseen classes')
# pred=knn_clf_gsl.predict(testData)
# print('unseen classes accuracy:')
# unseen_accuracy=accuracy_score(testLabels,pred)
# print(unseen_accuracy)
# # print(knn_clf_gsl.score(testData,testLabels))
# print('seen classes accuracy:')
# pred2=knn_clf_gsl.predict(see_test_feature)
# print('test labels')
# print(seen_test_labels)
# seen_accuracy=accuracy_score(seen_test_labels,pred2)
# print(seen_accuracy)
# # print(knn_clf_gsl.score(see_test_feature,seen_test_labels))
...
##########保存
import scipy.io
scipy.io.savemat('confusion.mat', mdict={'ls': testLabels ,'ps': pred, 'classes': allTestClasses})
