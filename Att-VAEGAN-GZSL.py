...
AWA=loadmat('../../Datasets/xlsa17/data/AWA1/AWA.mat')
testClasses=AWA["testclasses"]
trainClasses=AWA["trainclasses"]
...
#虚假的训练数据和标签及属性
pseudoTrainData_seen = []
pseudoTrainLabels_seen =[]
pseudoTrainAttr_seen = []
nSamples=10
#生成的训练未见类别数目10*200=20000
totalExs = len(trainClasses)*nSamples
##########啊哈哈哈哈
...
for tc in trainClasses:
    for ii in range(0,nSamples):
        pseudoTrainAttr_seen.append(ATTR[tc-1])
        pseudoTrainLabels_seen.append(tc)
pseudoTrainAttr_seen = np.array(pseudoTrainAttr_seen)
print(pseudoTrainAttr_seen)
print(pseudoTrainLabels_seen)
pseudoTrainLabels_seen = np.array(pseudoTrainLabels_seen)
#10000*312
#共10000个生成的样本[ 43  43  43 ... 168 168 168]
#解码拼接
dec_ip = np.concatenate((noise_gen, pseudoTrainAttr_seen) , axis=1)
###########################################################
#解码器进行预测得到的标签
pseudoTrainData_seen = decoder.predict(dec_ip)
# pseudoTrainData = min_max_scaler.fit_transform(pseudoTrainData)
print(pseudoTrainData_seen)
...
print(pseudoTrainData_seen.shape)
print(pseudoTrainLabels.shape)
print(pseudoTrainLabels_seen.shape)
print(pseudoTrainLabels)
print(pseudoTrainLabels_seen)
print(pseudoTrainAttr.shape)
print(pseudoTrainAttr_seen.shape)
pseudoTrainData=np.row_stack((pseudoTrainData,pseudoTrainData_seen))
# pseudoTrainLabels=np.row_stack((pseudoTrainLabels,pseudoTrainLabels_seen))

pseudoTrainAttr=np.row_stack((pseudoTrainAttr,pseudoTrainAttr_seen))
pseudoTrainLabels=[list(pseudoTrainLabels),list(pseudoTrainLabels_seen)]
...
#         tl.append(j)
# allTrainLabels=tl
pseudoTrainLabels = [ i for item in pseudoTrainLabels for i in item]
print(pseudoTrainLabels)
########################################################################
#                              训练AttGAN                              #
########################################################################
...
for i in range(0):
    ...
    for _ in tqdm(range(0)):
        ...
        #生成相应的Discriminator输出结果
        yGen = np.ones(batchSize)
        generatedImages_seen = generator.predict(att_noise)

        generatedImages_seen = min_max_scaler.fit_transform(generatedImages_seen)
        pseudoTrainData=np.row_stack((pseudoTrainData,generatedImages_seen))
        tl = []
        for i in label_x:
            for j in i:
                tl.append(j)
